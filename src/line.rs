...
pub fn tab_line(
    ...
    let builder = TabLineBuilder::new(config, mode_info.style.colors, cols);
...
impl TabLinePopulator {
    fn new(cols: usize, palette: Styling) -> Self {
        Self { cols, palette }
    }
    ...
    fn create_styled_indicator(&self, text: String, tab_index: usize) -> LinePart {
        ...
        let styled_parts = [style!(colors.text, colors.background).bold().paint(text)];
...
impl TabLinePrefixBuilder {
    ...
    fn build(&self, mode: InputMode) -> Vec<LinePart> {
        let mut parts = vec![];

        if let Some(mode_part) = self.create_mode_part(mode, 0) {
    ...
    fn create_mode_part(&self, mode: InputMode, used_len: usize) -> Option<LinePart> {
        let mode_text = format!(
            " {} ",
            match mode {
                InputMode::Session => "󰭖".to_owned(),
                InputMode::Pane => "".to_owned(),
                InputMode::Tab => "󰓩".to_owned(),
                InputMode::Scroll => "".to_owned(),
                InputMode::Search => "".to_owned(),
                InputMode::Locked => "".to_owned(),
                _ => format!("{:?}", mode)
                    .to_uppercase()
                    .chars()
                    .next()
...
impl RightSideElementsBuilder {
    ...
    fn build(&self, config: &TabLineConfig, available_space: usize) -> Vec<LinePart> {
        ...
        if let Some(session_name) = &config.session_name {
            if !&config.hide_session_name {
                elements.push(self.create_session_name_part(&session_name));
            }
        }
    ...
    fn create_session_name_part(&self, name: &str) -> LinePart {
    ...
    fn create_swap_layout_status(
        ...
        let indicator = self.create_swap_layout_styled_parts(
            &layout_name,
            config.mode,
    ...
    fn create_swap_layout_styled_parts(
        ...
        match mode {
            InputMode::Locked => style!(colors.bg, colors.fg)
                .italic()
                .paint(layout_name)
                .to_string(),
            _ if is_dirty => style!(colors.bg, colors.fg)
                .bold()
                .paint(layout_name)
                .to_string(),
            _ => style!(colors.bg, colors.green)
                .bold()
                .paint(layout_name)
                .to_string(),
        }
...
impl TabLineBuilder {
    pub fn new(config: TabLineConfig, palette: Styling, cols: usize) -> Self {
        Self {
            config,
            palette,
            cols,
    ...
    pub fn build(self, all_tabs: Vec<LinePart>, active_tab_index: usize) -> Vec<LinePart> {
        ...
        let mut prefix = prefix_builder.build(self.config.mode);
        ...
        let populator = TabLinePopulator::new(self.cols.saturating_sub(prefix_len), self.palette);
